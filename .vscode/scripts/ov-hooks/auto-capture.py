#!/usr/bin/env python3
"""
Stop hook — VS Code Copilot ↔ OpenViking.

Reads the session transcript, extracts incremental new turns since the last
capture, pushes them to a persistent OpenViking session (identified by
copilot-{session_id}), then commits. OV's own pipeline drives async archive
generation and memory extraction in the background.

Capture state (how many turns already sent) is persisted per session in
~/.ov-hook-state/ so re-invocations are incremental.

Equivalent to the Claude Code memory plugin auto-capture.mjs.
Timeout budget: 30s (set in ov-auto-capture.json).
"""
...
MAX_TURNS = 20          # max turns to push per Stop invocation
...
# Categorized by signal type for better filtering
TRIGGERS = {
    "decision": re.compile(
...
def log_error(msg: str, exc: Exception = None) -> None:
    ...
    try:
        ...
        log_file = log_dir / f"auto-capture.log"
        full_msg = f"{msg}\n"
        if exc:
            full_msg += f"  {type(exc).__name__}: {exc}\n{traceback.format_exc()}\n"
        log_file.write_text(log_file.read_text("utf-8") if log_file.exists() else "" + full_msg)
        print(f"[auto-capture] {msg}", file=sys.stderr)
    except Exception:
        pass
...
def save_state(session_id: str, state: dict) -> None:
    try:
        ...
        log_error(f"Failed to save state to {STATE_DIR}/{safe}.json", e)
...
def main():
    ...
    if not session_id or not transcript_path:
        log_error(f"Missing required fields: session_id={session_id}, transcript_path={transcript_path}")
    ...
    try:
        ...
        log_error(f"Failed to read transcript {transcript_path}", e)
        print_json(hook_continue())
        return
    ...
    if not all_turns:
        log_error(f"No turns extracted from {len(messages)} messages")
        print_json(hook_continue())
        return
    ...
    log_error(f"Stop event: {len(new_turns)} new turns → {len(filtered_turns)} after filter")
    
    if not filtered_turns:
        log_error(f"All {len(new_turns)} new turns filtered out (low signal)")
        print_json(hook_continue())
        return

    log_error(f"Session {session_id}: captured={already_captured}, new={len(new_turns)}, filtered={len(filtered_turns)}, total={len(all_turns)}")
    ...
    for turn in filtered_turns:
        ...
        result = add_message(cfg, ov_session_id, turn["role"], content, timeout=10.0)
        if result["ok"]:
            pushed_count += 1

    # Save state to prevent re-sending duplicates on next Stop
    save_state(session_id, {"captured_turn_count": len(all_turns)})
    log_error(f"Captured {pushed_count} turns → OV session {ov_session_id}")
...
if __name__ == "__main__":
    try:
        ...
        log_error("Unhandled exception in auto-capture", e)
        print_json(hook_continue())
